...
async function loadPage(pageName, element) {
    try {
        ...
        // KOMENTAR: Sekarang inisialisasi grafik dipindah ke halaman 'trip'
...
function initCharts() {
    ...
    // KOMENTAR: Speed sekarang Oranye
    speedChart = new Chart(ctxS, {
        type: 'line',
        data: { labels: Array(30).fill(''), datasets: [{ data: Array(30).fill(0), borderColor: '#d29922', fill: true, backgroundColor: 'rgba(210, 153, 34, 0.1)' }] },
        options: opt
    });

    // KOMENTAR: Current sekarang Biru (Cyan)
    currentChart = new Chart(ctxC, {
        type: 'line',
        data: { labels: Array(30).fill(''), datasets: [{ data: Array(30).fill(0), borderColor: '#58a6ff', fill: true, backgroundColor: 'rgba(88, 166, 255, 0.1)' }] },
        options: opt
    });
}
...
function updateUI(data) {
    ...
    // GAUGE ANIMATION (Start from 6 O'clock)
    ...
    const vals = {
        ...
        'trip-val': (data.trip?.km || 0).toFixed(1) + " km", 'range-val': (data.trip?.range || 0) + " km",
