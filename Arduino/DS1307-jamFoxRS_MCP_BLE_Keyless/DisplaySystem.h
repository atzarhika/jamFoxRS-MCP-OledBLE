...
void executeSettingAction() {
  ...
  if (settingsCursor == 0) { 
      soundEnabled = !soundEnabled; preferences.putBool("snd", soundEnabled); playBeep(100, 3500);
  } else if (settingsCursor == 1) { 
      buzzerIsPassive = !buzzerIsPassive; preferences.putBool("buzzPass", buzzerIsPassive); playBeep(150, 3500); 
      ...
      modePopupEnable = !modePopupEnable; preferences.putBool("popMode", modePopupEnable); playBeep(100, 3500);
  } else if (settingsCursor == 5) { 
      autoSleepEnable = !autoSleepEnable; preferences.putBool("slpMode", autoSleepEnable); playBeep(100, 3500);
  } else if (settingsCursor == 6) { 
      inSettingsMode = false; lastButtonPress = millis(); playBeep(100, 3000);
  }
  preferences.end();
}
...
void updateOLED() {
  ...
  if (keylessEnabled && inShutdownWarning) {
      ...
      int secondsLeft = (SHUTDOWN_GRACE_MS - elapsed) / 1000;
  ...
  if (standActive == 1) {
      if ((millis() / 450) % 2 == 0) { 
          display.setFont(&FreeSansBold9pt7b);
          showCenteredText("STAND!", 16);
          display.setFont(); 
          display.setTextSize(1);
          showCenteredText("LIFT TO RIDE", 24);
      } else {
          display.clearDisplay(); 
      }
      display.display();
  ...
  if (showCruisePopup) {
      if (millis() - lastCruiseChange < 2000) {
          display.setFont(&FreeSansBold9pt7b);
          showCenteredText("CRUISE", 16);
          display.setFont();
          display.setTextSize(1);
          showCenteredText("SPEED LOCKED", 24);
          display.display();
          return;
      } else {
  ...
  // JALAN KELUAR EVALUASI 1: Jika mode popup aktif, render mode berkendara melayang selama 2 detik,
  // setelah itu popup dilepas dan visual otomatis kembali merender halaman aktif terakhir sebelum popup.
  if (showModePopup) {
    if (millis() - lastModeChange < 2000) { display.setFont(&FreeSansBold9pt7b); showCenteredText(currentMode, 22); display.display(); return; } else { showModePopup = false; }
  }
  ...
  switch (currentPage) {
    case 1: { 
      ...
      display.setFont(); display.setTextSize(1); display.setCursor(88, 5);  display.print(DAY_NAMES[dt.dayOfTheWeek()]); 
      
      // EVALUASI 2: PERGANTIAN DINAMIS TIAP 2 DETIK ANTARA TANGGAL BULAN DENGAN TAHUN
      display.setCursor(88, 15);
      if ((millis() / 2000) % 2 == 0) {
          display.printf("%d %s", dt.day(), MONTH_NAMES[dt.month() - 1]);
      } else {
